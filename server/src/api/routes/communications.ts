...
router.post('/', async (req: Request, res: Response) => {
  try {
    const patientId =
      req.body.patientId ??
      (await prisma.patient.findFirst({ select: { id: true }, orderBy: { createdAt: 'asc' } }))?.id;

    if (!patientId) {
      res.status(400).json({ error: 'A patient is required to send a communication' });
      return;
    }

    if (!req.body.body || !req.body.channel) {
