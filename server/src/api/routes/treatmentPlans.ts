...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const requestedPatientId = planData.patientId as string | undefined;
    const requestedProviderId = planData.providerId as string | undefined;
    ...
    const providerId = requestedProvider?.id ?? fallbackProvider?.id;
    const totalEstimate = Number(planData.totalEstimate ?? planData.totalCost ?? 0);
    const insuranceEst = Number(planData.insuranceEst ?? planData.estimatedInsurance ?? 0);
    const patientEst = Number(planData.patientEst ?? planData.patientPortion ?? Math.max(0, totalEstimate - insuranceEst));

    if (!patientId || !providerId || !planData.title) {
      res.status(400).json({ error: 'patientId, providerId, and title are required' });
      return;
    }
    ...
    const plan = await prisma.treatmentPlan.create({
      data: {
        ...
        status: planData.status ?? 'proposed',
        presentedDate: planData.presentedDate ?? new Date().toISOString().split('T')[0],
        ...
        priority: planData.priority ?? 'standard',
        notes: planData.notes ?? planData.description ?? null,
        items: items
          ? {
              create: items.map((item: Record<string, unknown>, index: number) => ({
                procedureCode: String(item.procedureCode ?? ''),
                description: String(item.description ?? 'Treatment item'),
                toothNumber: item.toothNumber != null ? String(item.toothNumber) : item.toothNumbers != null ? String(item.toothNumbers) : null,
                surface: item.surface != null ? String(item.surface) : null,
                estimatedCost: Number(item.estimatedCost ?? item.cost ?? 0),
                insuranceCoverage: Number(item.insuranceCoverage ?? 0),
                patientCost: Number(item.patientCost ?? item.cost ?? 0),
