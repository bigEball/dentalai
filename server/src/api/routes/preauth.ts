...
router.post('/', async (req: Request, res: Response) => {
  try {
    const requestedPlanId = req.body.insurancePlanId as string | undefined;
    ...
    const requestedPatient = requestedPatientId
      ...
      : null;
    const fallbackPlan = await prisma.insurancePlan.findFirst({
      select: { id: true, patientId: true },
      orderBy: { createdAt: 'asc' },
    });
    const insurancePlanId = requestedPlan?.id ?? fallbackPlan?.id;
    const patientId = requestedPatient?.id ?? requestedPlan?.patientId ?? fallbackPlan?.patientId;
    const procedureCodes = req.body.procedureCodes ?? 'D2740';
    const estimatedCost = Number(req.body.estimatedCost ?? req.body.estimatedAmount ?? 0);
    const reason = req.body.reason ?? 'Demo pre-authorization draft';

    if (!patientId || !insurancePlanId) {
      res.status(400).json({ error: 'patientId and insurancePlanId are required' });
      return;
    }
    ...
    const preAuth = await prisma.preAuthorization.create({
      data: {
        ...
        toothNumbers: req.body.toothNumbers,
        reason,
